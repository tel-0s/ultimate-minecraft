...
pub struct World {
    ...
    /// Game-installed replaceability predicate (AIR-only default until
    /// the game installs one). See [`Self::set_replaceable_fn`].
    replaceable: std::sync::OnceLock<fn(BlockId) -> bool>,
...
impl World {
    pub fn new() -> Self {
        Self {
            ...
            replaceable: std::sync::OnceLock::new(),
            dirty: DashSet::new(),
            sky_lit: DashSet::new(),
        }
    }
    ...
    /// engine itself only knows AIR. Set once at startup — later calls
    /// are ignored (first installation wins).
    pub fn set_replaceable_fn(&self, f: fn(BlockId) -> bool) {
        let _ = self.replaceable.set(f);
    }

    /// Is this block displaceable by a materializing entity?
    pub fn is_replaceable(&self, id: BlockId) -> bool {
        match self.replaceable.get() {
            Some(f) => f(id),
            None => id == BlockId::AIR,
        }
    }
